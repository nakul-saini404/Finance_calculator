import { Box, Typography, Select, MenuItem, TextField, InputAdornment, Divider, useMediaQuery, useTheme } from '@mui/material';
  ...
  return (
    <Box sx={{ p: isMobile ? '16px 14px' : '30px 28px', overflowY: 'auto', height: 'calc(100vh - 56px)' }}>
      <PageHeader
        ...
        subtitle="Add / Remove GST · CGST · SGST · IGST · All rates — India 🇮🇳"
      ...
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 1.5 : 2 }}>
          <SectionCard title="GST Settings" subtitle="Select rate, enter amount, choose transaction type" accent="#fbbf24">
            ...
            {/* Rate label */}
            <Typography sx={{ fontSize: 11.5, fontWeight: 500, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: '0.8px', mb: 1.25 }}>
              GST Rate
            </Typography>

            {/* Rate Grid — 3-col on desktop, 3-col tight on mobile */}
            <Box sx={{
              display: 'grid',
              ...
              {GST_RATES.map((r) => (
                ...
                >
                  <Typography sx={{
                    ...
                    fontWeight: 700,
                    color: gstRate === r.rate ? '#fbbf24' : 'text.primary',
                  }}>
                    {r.rate === 'custom' ? '✏️' : `${r.rate}%`}
                  </Typography>
                  <Typography sx={{ fontSize: isMobile ? 9 : 10, color: gstRate === r.rate ? '#fbbf24' : 'text.disabled', mt: 0.25 }}>
                    {r.label}
                  </Typography>
                </Box>
              ))}
            ...
            <Box sx={{ mb: 2 }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 1 }}>
                ...
                <Box sx={{ background: '#1a1d2e', px: 1.25, py: 0.25, borderRadius: 1.5 }}>
                  <Typography sx={{ fontSize: 12, fontWeight: 700, fontFamily: "'Syne', sans-serif" }}>
          ...
          <SectionCard title="Common Items Reference" accent="#fbbf24">
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: isMobile ? 0.5 : 0.75, mt: 1.75 }}>
              {GST_COMMON_ITEMS.map((item) => (
                ...
                >
                  ...
                  <Typography sx={{
                    fontWeight: 600,
                    fontSize: isMobile ? 11.5 : 12.5,
                    color: item.rate === 0 ? '#34d399' : item.rate <= 12 ? '#60a5fa' : item.rate <= 18 ? '#fbbf24' : '#f87171',
                  }}>
                    {item.rate}%
                  </Typography>
                </Box>
        ...
        {/* RIGHT — Results Panel */}
        <Box sx={{ position: isMobile ? 'static' : 'sticky', top: 20 }}>
          <Box sx={{ background: '#0f1018', border: '1px solid rgba(255,255,255,0.06)', borderRadius: 4, p: isMobile ? 2 : 2.75 }}>
            <ResultCard
              label="GST Amount"
              value={fmtR(results.gstAmt)}
              color="#fbbf24"
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1.25, mb: 2 }}>
              {[
                ...
                <Box key={tile.label} sx={{ background: '#141520', borderRadius: 2.5, p: isMobile ? 1.25 : 1.75 }}>
                  <Typography sx={{ fontSize: 10, color: 'text.disabled', textTransform: 'uppercase', letterSpacing: '0.5px', mb: 0.75 }}>
                    {tile.label}
                  </Typography>
                  <Typography sx={{ fontFamily: "'Syne', sans-serif", fontSize: isMobile ? 15 : 18, fontWeight: 700, color: tile.color }}>
                    {tile.value}
                  </Typography>
                </Box>
              ))}
